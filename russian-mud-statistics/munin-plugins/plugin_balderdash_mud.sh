...
echo -n `date` " " >> /tmp2/mudstat.log1
curl http://tlkrus.rusfur.net:8880/who 2>/dev/null | grep ÍÁËÓÉÍÕÍ | awk '{print "king.value " substr($1,8)+0}' | tee -a /tmp2/mudstat.log1
echo -n `date` " " >> /tmp2/mudstat.log1
curl http://hiervard.ru/info/stat/online.shtml 2>/dev/null | grep Ñìåðòí | grep -v h5 | awk 'BEGIN {i=0} {i=$6+0} END {print "hiervard.value "i}' | tee -a /tmp2/mudstat.log1
cat /tmp2/adan.txt
cat /tmp2/arda.txt
