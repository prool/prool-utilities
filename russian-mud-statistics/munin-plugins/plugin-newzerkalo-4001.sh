...
if [ "$1" = "config" ]; then
    echo 'graph_title Zerkalo 4001 imms and players'
...
# cat /var/www/zerkalo.kharkov.org/wholist.html | grep -a "Всего" | awk '{print "zmud-players.value " $6}'
# cat /var/www/zerkalo.kharkov.org/wholist.html | grep -a "Всего" | awk '{print "zmud-imms.value " $4}'
if grep "Никого" /tmp2/wholist4001.html > /dev/null
then
